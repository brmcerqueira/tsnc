...
use melior::dialect::{arith, func, llvm, ods};
use melior::ir::attribute::{DenseI32ArrayAttribute, FlatSymbolRefAttribute, IntegerAttribute, StringAttribute, TypeAttribute};
use melior::ir::{Attribute, Block, BlockLike, Identifier, Location, Module as MlirModule, Operation, Region, RegionLike, Type, Value};
use std::collections::HashMap;
use melior::dialect::func::{func, r#return};
use melior::dialect::llvm::r#type::{array, function, pointer};
use melior::dialect::ods::llvm::GlobalOperation;
use melior::ir::operation::OperationBuilder;
use melior::ir::r#type::{FunctionType, IntegerType};
use swc_ecma_ast::{BinaryOp, Callee, Decl, Expr, ExprOrSpread, FnDecl, Lit, MemberProp, Module, ModuleItem, Pat, Stmt, TsKeywordTypeKind, TsType};

pub struct Compiler<'c> {
    context: &'c Context,
    location: Location<'c>,
    pub(crate) mlir_module: MlirModule<'c>,
    i64_type: Type<'c>,
    i32_type: Type<'c>,
    ptr_type: Type<'c>,
...
impl<'c> Compiler<'c> {
    ...
    pub(crate) fn compile_module(&mut self, module: &Module) -> anyhow::Result<()> {
    ...
    fn compile_main_entry(&mut self, stmts: &[&Stmt]) -> anyhow::Result<()> {
        let block = Block::new(&[]);
    ...
    fn compile_function(&mut self, function: &FnDecl) -> anyhow::Result<()> {
